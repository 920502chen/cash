...
DB_PATH = "database.db"
CATEGORIES = ["餐飲", "交通", "網購", "休閒", "醫療", "退款", "購物", "待分類"]
...
def normalize_category(category: Any, amount: float | int | str = 0) -> str:
    ...
    return cat if cat in CATEGORIES else "待分類"
...
def rows_from_imported_file(uploaded_file) -> List[Dict[str, Any]]:
    ...
    for _, item in df.iterrows():
        ...
        if pd.isna(spend_date):
            spend_date_str = date.today().isoformat()
        else:
            spend_date_str = spend_date.date().isoformat()
    ...
    for r in rows:
        cur.execute(
            ...
            (
                ...
                normalize_category(r.get("category", "待分類"), float(r.get("amount") or 0)),
                r.get("merchant") or "",
                source_file,
                now,
            ),
...
3. 若圖片有多筆交易，請輸出多筆。
4. 分類只能是：餐飲、交通、網購、休閒、醫療、退款、購物、待分類。
...
"""
    ...
    try:
        ...
        for item in data:
            ...
            if amount == 0:
                continue
            cat = normalize_category(item.get("category", "待分類"), amount)
...
st.caption("支援 CSV / Excel。欄位可用：日期/消費日期/交易日期、金額、內容/消費內容/摘要、分類、商家。負數會自動歸類為退款。")
...
positive_df = month_df[(month_df["amount"] > 0) & (~month_df["category"].isin(PIE_EXCLUDED_CATEGORIES))].copy()
if positive_df.empty:
    ...
    st.caption("圓餅圖只統計正向消費，且排除退款類別；退款會保留在明細並計入本月淨額。")
    st.plotly_chart(fig, use_container_width=True)

st.subheader(f"{selected_month} 明細")
st.caption("可直接在表格中修改資料；勾選「刪除」後按下方按鈕即可刪除，不需要手動輸入交易 ID。")
