...
def request_confirm(action_key: str, message: str):
    ...
    st.warning(message)
    c1, c2 = st.columns(2)
    confirmed = c1.button("確認刪除", type="primary", use_container_width=True, key=f"confirm_{action_key}")
    cancelled = c2.button("取消", use_container_width=True, key=f"cancel_{action_key}")
