...
def find_google_drive_file(service, filename: str, folder_id: str = "") -> Optional[Dict[str, Any]]:
    """尋找備份檔；使用目前 OAuth 帳號及指定資料夾。"""
...
def download_google_drive_file_bytes(service, file_id: str) -> bytes:
...
def cloud_backup_status() -> Dict[str, Any]:
    ...
    if service is None:
        return {"connected": False, "transactions": None, "settings": None}
    folder_id = get_secret("GDRIVE_FOLDER_ID")
    return {
        "connected": True,
        "transactions": find_google_drive_file(service, GDRIVE_BACKUP_FILENAME, folder_id),
        "settings": find_google_drive_file(service, GDRIVE_SETTINGS_FILENAME, folder_id),
    }


def replace_transactions_from_csv_bytes(csv_bytes: bytes) -> int:
    """以雲端 CSV 完整取代本機交易。"""
    ...
    missing = required - set(df.columns)
    if missing:
        raise ValueError(f"雲端交易檔缺少必要欄位：{', '.join(sorted(missing))}")
    ...
    for _, item in df.iterrows():
        ...
        if merchant.lower() == "nan":
            merchant = ""
        source_file = str(item.get("source_file", "Google Drive 還原") or "Google Drive 還原").strip()
        if source_file.lower() == "nan":
            source_file = "Google Drive 還原"
        category = normalize_category(item.get("category", "待分類"), amount)
        created_at = str(item.get("created_at", now) or now)
        if created_at.lower() == "nan":
            created_at = now
        rows.append((spend_date_str, amount, description, category, merchant, source_file, created_at))
...
def replace_settings_from_json_bytes(settings_bytes: bytes) -> Dict[str, int]:
    """以雲端設定檔取代自訂分類與關鍵字規則。"""
    data = json.loads(settings_bytes.decode("utf-8-sig"))
    categories = [str(x).strip() for x in (data.get("categories", []) or []) if str(x).strip()]
    rules = data.get("keyword_rules", []) or []
    now = datetime.now().isoformat(timespec="seconds")
    ...
    try:
        cur = conn.cursor()
        cur.execute("DELETE FROM keyword_rules")
        cur.execute("DELETE FROM categories")
        merged_categories = []
        for cat in DEFAULT_CATEGORIES + categories:
            if cat not in merged_categories:
                merged_categories.append(cat)
        cur.executemany(
            "INSERT OR IGNORE INTO categories (name, created_at) VALUES (?, ?)",
            [(cat, now) for cat in merged_categories],
        )
        valid_categories = set(merged_categories)
        added_rules = 0
        seen = set()
        for item in rules:
            if not isinstance(item, dict):
        ...
        conn.close()
    return {"categories": len(merged_categories), "keyword_rules": added_rules}


def restore_from_google_drive(restore_transactions: bool = True, restore_settings: bool = True, show_success: bool = True) -> Dict[str, Any]:
    """從 Google Drive 下載並還原備份。還原本身不會立刻反向覆蓋雲端。"""
    service = build_drive_service()
    if service is None:
        if show_success:
            st.warning("尚未連結 Google Drive，無法還原。")
        return {"ok": False}

    folder_id = get_secret("GDRIVE_FOLDER_ID")
    result: Dict[str, Any] = {"ok": True, "transactions": 0, "settings": None, "missing": []}
    try:
        tx_bytes = None
        settings_bytes = None
        if restore_transactions:
            tx_file = find_google_drive_file(service, GDRIVE_BACKUP_FILENAME, folder_id)
            if tx_file:
                tx_bytes = download_google_drive_file_bytes(service, tx_file["id"])
                result["transactions"] = replace_transactions_from_csv_bytes(tx_bytes)
            else:
                result["missing"].append(GDRIVE_BACKUP_FILENAME)

        if restore_settings:
            settings_file = find_google_drive_file(service, GDRIVE_SETTINGS_FILENAME, folder_id)
            if settings_file:
                settings_bytes = download_google_drive_file_bytes(service, settings_file["id"])
                result["settings"] = replace_settings_from_json_bytes(settings_bytes)
            else:
                result["missing"].append(GDRIVE_SETTINGS_FILENAME)

        # 將目前本機內容視為已與雲端同步，避免還原後又立即反向備份。
        if tx_bytes is not None or settings_bytes is not None:
            tx_hash = current_transactions_hash()
            settings_hash = current_settings_hash()
            combined_hash = hashlib.sha256((tx_hash + settings_hash).encode("utf-8")).hexdigest()
            set_setting("last_gdrive_backup_hash", combined_hash)
            set_setting("last_gdrive_transactions_hash", tx_hash)
            set_setting("last_gdrive_settings_hash", settings_hash)
            set_setting("last_gdrive_restore_at", datetime.now().isoformat(timespec="seconds"))
            set_setting("backup_dirty", "0")
            set_setting("backup_dirty_reason", "")

        if show_success:
            parts = []
            if restore_transactions and GDRIVE_BACKUP_FILENAME not in result["missing"]:
                parts.append(f"交易 {result['transactions']} 筆")
            if restore_settings and GDRIVE_SETTINGS_FILENAME not in result["missing"]:
                settings_result = result.get("settings") or {}
                parts.append(f"分類 {settings_result.get('categories', 0)} 個、關鍵字規則 {settings_result.get('keyword_rules', 0)} 筆")
            if parts:
                st.success("Google Drive 還原成功：" + "；".join(parts) + "。")
            if result["missing"]:
                st.warning("雲端找不到：" + "、".join(result["missing"]))
        return result
    except HttpError as e:
        result["ok"] = False
        if show_success:
            st.error("Google Drive 還原失敗。以下是 Google 回傳的原始錯誤：")
            try:
        ...
        result["ok"] = False
        if show_success:
            st.error(f"Google Drive 還原失敗：{type(e).__name__}")
            st.exception(e)
        return result


def local_transactions_empty() -> bool:
    conn = get_conn()
    try:
...
def maybe_auto_restore_from_google_drive() -> None:
    """每個瀏覽器工作階段只檢查一次；本機無交易時才自動從雲端還原。"""
    if st.session_state.get("cloud_restore_checked"):
        return
    if not get_google_credentials():
        return
    st.session_state["cloud_restore_checked"] = True
    if not local_transactions_empty():
        return
    with st.spinner("本機沒有交易，正在檢查 Google Drive 備份…"):
        result = restore_from_google_drive(True, True, show_success=False)
    if result.get("ok") and (result.get("transactions", 0) > 0 or result.get("settings")):
        st.toast(f"已從 Google Drive 自動還原 {result.get('transactions', 0)} 筆交易與設定。", icon="☁️")
        st.rerun()
...
def render_google_drive_backup_panel():
    ...
    if not google_oauth_ready():
        ...
        st.caption("請先完成 Google Drive 連線設定。")
        return
    ...
    st.caption("資料變更後會自動備份。")
    if st.button("立即備份到 Google Drive", use_container_width=True):
        backup_transactions_to_google_drive(show_success=True, force=True)

    st.divider()
    st.subheader("從 Google Drive 還原")
    restore_choice = st.radio(
        "還原內容",
        ["全部（交易＋設定）", "只還原交易明細", "只還原分類與關鍵字設定"],
        key="gdrive_restore_choice",
        label_visibility="collapsed",
    )
    if st.button("從 Google Drive 還原", use_container_width=True):
        st.session_state["confirm_restore_from_gdrive"] = True
    if st.session_state.get("confirm_restore_from_gdrive"):
        st.warning("還原會以雲端備份覆蓋你選擇的本機資料。確定要繼續嗎？")
        r1, r2 = st.columns(2)
        if r1.button("確認還原", type="primary", use_container_width=True, key="confirm_gdrive_restore_btn"):
            restore_tx = restore_choice in ["全部（交易＋設定）", "只還原交易明細"]
            restore_cfg = restore_choice in ["全部（交易＋設定）", "只還原分類與關鍵字設定"]
            with st.spinner("正在從 Google Drive 下載並還原…"):
                result = restore_from_google_drive(restore_tx, restore_cfg, show_success=True)
            st.session_state["confirm_restore_from_gdrive"] = False
            if result.get("ok"):
                st.rerun()
        if r2.button("取消", use_container_width=True, key="cancel_gdrive_restore_btn"):
            st.session_state["confirm_restore_from_gdrive"] = False
            st.rerun()
...
maybe_auto_restore_from_google_drive()
CATEGORIES = get_categories()

# 安全優先備份：資料變更後會立即同步；不再依賴離開頁面或定時自動備份。
# 保留手動立即備份按鈕，若先前有未備份變更可隨時補同步。

st.title("🧾 AI 帳單記帳助手｜Gemini + Google Drive 雙向同步版")
st.caption("上傳帳單/發票/收據圖片後自動辨識交易；可編輯明細、設定關鍵字分類。資料變更完成後會立即備份；本機沒有交易時會自動從 Google Drive 還原交易 CSV 與設定檔 JSON。")
...
if positive_df.empty:
    ...
    fig.update_traces(
        hovertemplate="%{label}<br>金額：$%{value:,.0f}<br>比例：%{percent}<extra></extra>"
    )
    st.caption("點擊圓餅圖中的分類，可直接查看該分類的交易明細。只統計正向消費，並排除退款類別。")

    chart_event = st.plotly_chart(
        fig,
        use_container_width=True,
        key=f"category_pie_{selected_month}",
        on_select="rerun",
        selection_mode="points",
    )

    selected_category = st.session_state.get(f"selected_pie_category_{selected_month}")
    try:
        points = chart_event.selection.points if chart_event and chart_event.selection else []
    except Exception:
        try:
            points = chart_event.get("selection", {}).get("points", []) if chart_event else []
        except Exception:
            points = []
    ...
    if points:
        ...
        if isinstance(point, dict):
            ...
            if isinstance(customdata, (list, tuple)) and customdata:
                clicked_category = customdata[0]
            clicked_category = clicked_category or point.get("label")
        else:
            customdata = getattr(point, "customdata", None)
            if isinstance(customdata, (list, tuple)) and customdata:
                clicked_category = customdata[0]
            clicked_category = clicked_category or getattr(point, "label", None)
        if clicked_category:
            selected_category = str(clicked_category)
            st.session_state[f"selected_pie_category_{selected_month}"] = selected_category

    if selected_category:
        st.markdown('<div id="category-detail-table"></div>', unsafe_allow_html=True)
        detail_title_col, clear_filter_col = st.columns([4, 1])
        detail_title_col.subheader(f"{selected_category}｜交易明細")
        if clear_filter_col.button(
            "清除篩選",
            key=f"clear_pie_filter_{selected_month}",
            use_container_width=True,
        ):
            st.session_state.pop(f"selected_pie_category_{selected_month}", None)
            st.rerun()

        category_detail_df = live_df[live_df["category"] == selected_category].copy()
        category_detail_df = category_detail_df.sort_values("spend_date", ascending=False)
        category_detail_df = category_detail_df[
            ["spend_date", "amount", "description", "merchant", "source_file"]
        ].rename(
            columns={
                "spend_date": "日期",
                "amount": "金額",
        ...
        st.caption(
            f"共 {len(category_detail_df)} 筆，合計 ${category_detail_df['金額'].sum():,.0f}。"
        )
        components.html(
            """
            <script>
            setTimeout(function () {
              try {
                const target = window.parent.document.getElementById('category-detail-table');
