...
def mark_backup_dirty(reason: str = "資料已變更"):
    """只標記待備份，不立刻呼叫 Google Drive，避免每個操作都卡住。"""
    ...
    # 這次操作剛完成後的 rerun 先不要自動備份，讓畫面先快速回應。
    st.session_state["skip_auto_backup_once"] = True


def backup_after_change():
    """資料變更後只標記待備份；實際上傳交給延遲智慧備份或手動備份。"""
    mark_backup_dirty()
...
def render_google_drive_backup_panel():
    ...
    if dirty:
        ...
        st.warning(f"有尚未備份的變更（{dirty_at}）。")
    else:
        st.caption("目前沒有待備份變更。")
    st.caption(f"最終智慧備份：操作會先立即完成、不等 Google Drive；有未備份變更時每 1 分鐘自動檢查，離開頁面時也會嘗試最後備份。")
...
# 最終智慧備份：資料變更先標記 dirty；有 dirty 時每 1 分鐘自動檢查，離開頁面也會嘗試最後同步。
setup_dirty_autorefresh()
render_beforeunload_backup_hook()
maybe_auto_backup_to_google_drive()

st.title("🧾 AI 帳單記帳助手｜Gemini + Google Drive 最終智慧備份版")
st.caption("上傳帳單/發票/收據圖片後自動辨識交易；可編輯明細、設定關鍵字分類，並以最終智慧備份同步交易 CSV 與設定檔 JSON 到個人 Google Drive：操作不阻塞、每 1 分鐘檢查，離開頁面時也會嘗試補備份。")
