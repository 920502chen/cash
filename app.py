...
def render_category_keyword_settings(location: str = "main"):
    ...
    st.subheader("分類設定")
    st.caption("可新增自己的分類；刪除分類時，原本屬於該分類的交易會改成「待分類」。")
    ...
    with st.form(f"add_rule_form_{location}", clear_on_submit=True):
        keyword = st.text_input("關鍵字", placeholder="例如：星巴克、Uber、蝦皮、Netflix", key=f"keyword_{location}")
        ...
        if st.form_submit_button("新增關鍵字規則", use_container_width=True):
            if add_keyword_rule(keyword, rule_cat):
                ...
                st.warning("關鍵字不可空白，或這個關鍵字已存在。")

    rules_df = load_keyword_rules()
    if rules_df.empty:
        st.info("目前尚未設定關鍵字規則。")
    else:
        show_rules = rules_df.rename(columns={"id": "ID", "keyword": "關鍵字", "category": "分類"})
        st.dataframe(show_rules, hide_index=True, use_container_width=True)
        rule_ids = rules_df["id"].tolist()
        selected_rule = st.selectbox("選擇要刪除的規則 ID", rule_ids, key=f"selected_rule_{location}")
        if st.button("刪除關鍵字規則", use_container_width=True, key=f"delete_rule_btn_{location}"):
