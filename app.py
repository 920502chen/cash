...
def render_google_drive_backup_panel():
    ...
    st.caption("智慧備份：每次打開網頁會自動檢查交易與設定檔，只有內容有變更才會重新上傳。")
...
# 智慧備份：每次開啟或重新整理時檢查交易與設定檔是否變更。
# 只有內容與上次備份不同時才會上傳，避免重複消耗 Google API。
if get_google_credentials():
    backup_transactions_to_google_drive(show_success=False, force=False)

st.title("🧾 AI 帳單記帳助手｜Gemini + Google Drive 智慧備份版")
st.caption("上傳帳單/發票/收據圖片後自動辨識交易；可編輯明細、設定關鍵字分類，並智慧備份交易 CSV 與設定檔 JSON 到個人 Google Drive。")
