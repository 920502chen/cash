...
def backup_transactions_to_google_drive(show_success: bool = True) -> bool:
    ...
    try:
        ...
        query = f"name='{GDRIVE_BACKUP_FILENAME}' and trashed=false{parent_filter}"
        ...
        metadata = {"name": GDRIVE_BACKUP_FILENAME, "mimeType": "text/csv"}
        if folder_id:
            metadata["parents"] = [folder_id]
        if files:
            file_id = files[0]["id"]
            service.files().update(fileId=file_id, media_body=media, fields="id", supportsAllDrives=True).execute()
        else:
...
def backup_after_change():
    ...
    if get_google_credentials():
        backup_transactions_to_google_drive(show_success=False)
...
def render_google_drive_backup_panel():
    ...
    if st.button("立即備份到 Google Drive", use_container_width=True):
        backup_transactions_to_google_drive(show_success=True)
...
# 每次開啟網頁且已授權時，自動備份一次。
if get_google_credentials() and not st.session_state.get("auto_gdrive_backup_done"):
    backup_transactions_to_google_drive(show_success=False)
    st.session_state["auto_gdrive_backup_done"] = True

st.title("🧾 AI 帳單記帳助手｜Gemini + Google Drive OAuth 版")
st.caption("上傳帳單/發票/收據圖片後自動辨識交易；可編輯明細、設定關鍵字分類，並備份 CSV 到個人 Google Drive。")
