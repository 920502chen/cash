...
def mark_drive_backup_needed():
    st.session_state["drive_backup_needed"] = True


def get_drive_settings():
    try:
        service_json = st.secrets.get("GDRIVE_SERVICE_ACCOUNT_JSON", "")
    except Exception:
        service_json = ""
    try:
        ...
        folder_id = ""
    return str(service_json or "").strip(), str(folder_id or "").strip()


def is_drive_backup_configured() -> bool:
    service_json, folder_id = get_drive_settings()
    return bool(service_json and folder_id and service_account and build and MediaIoBaseUpload)


def backup_transactions_to_google_drive(show_message: bool = True) -> bool:
    service_json, folder_id = get_drive_settings()
    if not service_json or not folder_id:
        if show_message:
            st.warning("尚未設定 Google Drive 備份。請在 Streamlit Secrets 加入 GDRIVE_SERVICE_ACCOUNT_JSON 與 GDRIVE_FOLDER_ID。")
        return False
    if not (service_account and build and MediaIoBaseUpload):
        if show_message:
            st.error("尚未安裝 Google Drive 備份套件。請確認 requirements.txt 已包含 google-api-python-client 與 google-auth。")
        return False
    ...
    try:
        ...
        # 先確認資料夾真的能被 Service Account 看到、且可新增檔案。
        ...
        if folder_meta.get("mimeType") != "application/vnd.google-apps.folder":
            raise ValueError("GDRIVE_FOLDER_ID 指到的不是 Google Drive 資料夾")
        if folder_meta.get("capabilities", {}).get("canAddChildren") is False:
            raise PermissionError("Service Account 對此資料夾沒有新增檔案權限，請確認共用權限是編輯者")

        csv_bytes = load_transactions().to_csv(index=False).encode("utf-8-sig")
        media = MediaIoBaseUpload(io.BytesIO(csv_bytes), mimetype="text/csv", resumable=False)
        file_name = "transactions_backup.csv"

        query = (
            f"name='{file_name}' and '{folder_id}' in parents "
            "and trashed=false and mimeType='text/csv'"
        )
        found = drive.files().list(q=query, fields="files(id, name)", supportsAllDrives=True, includeItemsFromAllDrives=True).execute()
        ...
        if files:
            ...
            metadata = {"name": file_name, "parents": [folder_id], "mimeType": "text/csv"}
            drive.files().create(
                body=metadata,
                media_body=media,
                fields="id, modifiedTime",
        ...
        st.session_state["last_drive_backup_at"] = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        st.session_state["drive_backup_needed"] = False
        if show_message:
            st.success("已備份全部交易到 Google Drive：transactions_backup.csv")
        return True
        ...
        if show_message:
            ...
            try:
                if HttpError is not None and isinstance(e, HttpError):
                    detail = e.content.decode("utf-8", errors="ignore") if getattr(e, "content", None) else str(e)
            except Exception:
                detail = str(e)
            with st.expander("查看 Google Drive 原始錯誤訊息"):
                st.code(detail[:4000])
            st.info("這版已改用完整 Drive 權限 scope，並會先檢查資料夾是否可寫入。若仍失敗，請把上方原始錯誤訊息中的 reason / message 貼給我。")
        return False
...
# 每個瀏覽器工作階段第一次打開網頁時自動備份；資料有異動時也會在下一次重跑自動備份。
if is_drive_backup_configured():
    if not st.session_state.get("drive_backup_done_this_session") or st.session_state.get("drive_backup_needed"):
        backup_transactions_to_google_drive(show_message=False)
        st.session_state["drive_backup_done_this_session"] = True
...
with st.sidebar:
    ...
    if is_drive_backup_configured():
        st.success("已啟用 Google Drive 自動備份")
        if st.session_state.get("last_drive_backup_at"):
            st.caption(f"上次備份：{st.session_state['last_drive_backup_at']}")
        if st.button("立即備份到 Google Drive", use_container_width=True):
            backup_transactions_to_google_drive(show_message=True)
    else:
        st.warning("尚未啟用 Google Drive 自動備份")
