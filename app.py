...
def preprocess_image(uploaded_file) -> Image.Image:
    ...
    舊版會把所有圖片壓到 1800x1800，遇到信用卡明細長截圖時，上半段文字容易被壓縮到太小，
    Gemini 也可能只看中段或後段。新版只限制寬度，不硬壓高度，並保留長圖細節。
    """
...
def split_long_image(image: Image.Image, segment_height: int = 1200, overlap: int = 120) -> List[Image.Image]:
    """把長截圖切成多段，提高信用卡明細/交易列表辨識率。

    overlap 用來避免交易剛好落在切割線時被截斷；後續會再做去重。
    """
    if image.height <= segment_height + overlap:
        return [image]

    segments: List[Image.Image] = []
    y = 0
    while y < image.height:
        bottom = min(image.height, y + segment_height)
        seg = image.crop((0, y, image.width, bottom))
        segments.append(seg)
...
def dedupe_transactions(rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for r in rows:
        key = (
            str(r.get("spend_date", ""))[:10],
            round(float(r.get("amount", 0) or 0), 2),
            re.sub(r"\s+", "", str(r.get("description", "")).lower())[:40],
            re.sub(r"\s+", "", str(r.get("merchant", "")).lower())[:30],
        )
        if key in seen:
            continue
        seen.add(key)
...
def analyze_bill(image: Image.Image, segment_note: str = "") -> List[Dict[str, Any]]:
    api_key = get_api_key()
    if not api_key:
...
def analyze_bill_with_segments(image: Image.Image) -> List[Dict[str, Any]]:
    """長圖自動切割後逐段辨識，再合併去重。"""
    segments = split_long_image(image)
    if len(segments) == 1:
        return analyze_bill(image)

    st.info(f"偵測到長截圖，已自動切成 {len(segments)} 段分段辨識，避免漏掉上半部或下半部交易。")
    all_rows: List[Dict[str, Any]] = []
    progress = st.progress(0)
    for idx, seg in enumerate(segments, start=1):
        note = f"這是長截圖的第 {idx}/{len(segments)} 段。請只辨識本段內完整可見的交易；若交易被切到只剩半行、日期或金額不完整，請忽略，避免重複或錯誤。"
        rows = analyze_bill(seg, segment_note=note)
        all_rows.extend(rows)
        progress.progress(idx / len(segments))
    progress.empty()
    return dedupe_transactions(all_rows)
    ...
    if st.button("開始辨識並新增交易", type="primary"):
        ...
        for f in uploaded_files:
            with st.spinner(f"正在辨識：{f.name}"):
                ...
                rows = analyze_bill_with_segments(img)
                if rows:
